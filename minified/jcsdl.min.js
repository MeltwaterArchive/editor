...
(c=b,b=this.element,d=this.widget());f.each(c,function(c,u){function g(){return a||!0!==e.options.disabled&&!f(this).hasClass("ui-state-disabled")?("string"==typeof u?e[u]:u).apply(e,arguments):h}"string"!=typeof u&&(g.guid=u.guid=u.guid||g.guid||f.guid++);var k=c.match(/^(\w+)\s*(.*)$/),m=k[1]+e.eventNamespace;(k=k[2])?d.delegate(k,m,g):b.bind(m,g)})},_off:function(a,b){b=(b||"").split(" ").join(this.eventNamespace+" ")+this.eventNamespace;a.unbind(b).undelegate(b)},_delay:function(a,b){var c=this;
...
a){var c=this,d={},f=[],h="";e.each(b,function(a,b){var e=c.filterToCSDL(b);if(!e)return!0;d[b.id+""]=e});if("AND"!=a&&"OR"!=a){var h=this.tokenizeLogicString(a),l="";this.validateLogic(h,b);e.each(h,function(a,b){if("number"===typeof b)if(l.length&&f.push(e.trim(l)),l="",d[b+""]!==j)f.push(d[b+""]);else throw new JCSDL.LogicValidationException("A filter with ID '"+b+"' doesn't exist! You cannot use it in a logical expression",a);else l="&"==b||"|"==b?l+("|"==b?" OR ":" AND "):"!"===b?l+" NOT ":l+
...
b.operator).click():this.$view.find(".jcsdl-filter-value-input-operators .operator-"+b.operator).click();var d=this.$view.find(".jcsdl-filter-value-input-field");this.setValueForField(d,c,b.value,b.operator);b.cs&&this.$view.find(".jcsdl-operator-cs").click()},addFilterStep:function(b,a,c){var d=this;c=c===j?!0:c;var f=this.getTemplate("step"),h=this.steps.length;f.html(a).data("number",h).addClass("jcsdl-filter-step-number-"+h).data("name",b).addClass("jcsdl-filter-step-"+b).appendTo(this.$steps);
"target"==b&&f.jcsdlCarousel({select:function(){var a=e(this);f.addClass("selected").find(".jcsdl-carousel-item.selected").removeClass("selected");a.addClass("selected");d.didSelectTarget(a.data("name"))}});"field"==b&&f.jcsdlCarousel({select:function(){var b=e(this);d.$steps.find(".jcsdl-filter-step-target").addClass("field-selected");var c=f.data("number")-1;d.$steps.find(".jcsdl-filter-step-field.jcsdl-filter-step-number-"+c).addClass("field-selected");f.addClass("selected").find(".jcsdl-carousel-item.selected").removeClass("selected");
...
function(b,d){var f=e(d);0==b&&(a=f.data("name"));f.remove()});return a},didSelectTarget:function(b){if(this.definition.targets[b]===j)return!1;this.target=b;this.path=[];var a=this.removeFilterStepsAfterPosition(0),c=this.createFieldSelectionView(this.definition.targets[b].fields);this.addFilterStep("field",c,"field"!=a);this.trigger("targetSelect",[b])},didSelectField:function(b,a,c){var d=a.data("position");this.path.splice(d,this.path.length-d);c.removeClass("selected-final").find(".jcsdl-target-help").remove();
var e=this.getFieldInfoAtCurrentPath(b);!1!=e&&(d=this.removeFilterStepsAfterPosition(a.closest(".jcsdl-step").data("number")),this.path.push(b),e.fields!==j?(a=this.createFieldSelectionView(e.fields),this.addFilterStep("field",a,"field"!=d)):"string"!==typeof e.input?(c=this.createInputSelectionView(e.input),this.addFilterStep("input",c,"input"!=d)):(b=this.createValueInputView(e),this.addFilterStep("value",b,"value"!=d),this.addTargetHelpToStep(c),this.trigger("targetSelect",[this.getCurrentPath()])))},
...
a.getFieldInfoAtCurrentPath();var d=a.target,f=a.parser.getTargetInfo(d);path=a.path.join("-");title=f.name;cPath=[];e.each(a.path,function(b,c){cPath.push(c);title+=" &raquo; "+a.parser.getFieldInfo(d,cPath).name});var h=path;"augmentation"!=d&&(h=d+"."+h);b=a.config.targetHelpUrl.apply(a,[h.replace(/-/g,"."),b]);var l=e.jcsdlPopup({title:title});a.jsonpCache.targets[h]===j?e.ajax({url:b,type:"GET",async:!1,jsonpCallback:"jcsdlJSONP",contentType:"application/json",dataType:"jsonp",success:function(b){!b||
!b.html||!e.trim(b.html)?(l.setContent("<p>No documentation available.</p>"),l.reposition()):(l.setContent(b.html),l.reposition(),a.jsonpCache.targets[h]=b.html)},error:function(){l.setContent("<p>No documentation available.</p>");l.reposition()}}):(l.setContent(a.jsonpCache.targets[h]),l.reposition())})},didSubmitFilter:function(){var b=this.getFilterInfo();b&&this.gui.didSubmitFilter(b)},showError:function(b,a){this.$view.find(".jcsdl-error").remove();var c=this.getTemplate("error");c.find("span").html(b);
c.prependTo(this.$view.find(".jcsdl-footer")).hide().fadeIn();console!==j&&console.error(b,arguments)},trigger:function(b,a){this.gui.trigger(b,a)},createTargetSelectView:function(){var b=this,a=this.getTemplate("target"),c=a.find(".jcsdl-filter-target");e.each(this.definition.targets,function(a,e){b.createOptionForTarget(a,e).appendTo(c)});return a},createOptionForTarget:function(b,a){var c=this.getTemplate("targetOption").data("name",b).data("target",a).addClass("target-"+b);0<=e.inArray(b,this.config.hideTargets)&&
(c.addClass("jcsdl-option-hidden"),(this.filter.target===j||this.filter.target!==b)&&c.hide());return c},createFieldSelectionView:function(b){var a=this,c=this.path.length,d=this.target+(0<this.path.length?"."+this.path.join("."):""),f=this.getTemplate("field"),h=!1,j=e.merge([this.target],this.path),g="";f.data("position",c).addClass("field-select-"+c);e.each(j,function(b,c){g=g+(0<b?".":"")+c;if(0<=e.inArray(g,a.config.hideTargets))return h=!0});var m=f.find(".jcsdl-filter-target-field");e.each(b,
function(b,c){a.createOptionForField(b,c,d,h).appendTo(m)});return f},createOptionForField:function(b,a,c,d){var f=this;a=this.getTemplate("fieldOption").data("name",b).data("field",a).html(a.name).addClass("icon-"+this.getIconForField(b,a)).addClass("field-"+b);if(!d){var h=c+"."+b;if(0<=e.inArray(h,f.config.hideTargets))d=!0;else{var h=h.split("-"),l="";e.each(h,function(a,b){l+=(0<a?".":"")+b;if(0<=e.inArray(l,f.config.hideTargets))return d=!0})}}d&&a.addClass("jcsdl-option-hidden");d&&this.filter&&
this.filter.target!==j&&this.filter.fieldPath!==j&&this.filter.target+"."+this.filter.fieldPath.join(".").replace(/-/g,".")==c+"."+b&&(d=!1);d&&a.hide();return a},createInputSelectionView:function(b){var a=this,c=this.getTemplate("inputSelect"),d=c.find(".jcsdl-filter-target-field-input");e.each(b,function(b,c){a.createOptionForInput(c).appendTo(d)});return c},createOptionForInput:function(b){return this.getTemplate("inputSelectOption").data("name",b).addClass("icon-"+b+" input-"+b).html(b)},createValueInputView:function(b,
a){a=a||b.input;var c=this.getTemplate("valueInput");if(this.inputs[a]===j)return c;var d=this.inputs.exec(a,"init",[b]);d.find("input:first");var d=c.find(".jcsdl-filter-value-input-field").data("inputType",a).html(d),e=c.find(".jcsdl-filter-value-input-operators");"text"==a?this.createTextOperatorsSelectView(e,b,a,d):this.createOperatorsSelectView(e,b,a,d);return c},createOperatorsSelectView:function(b,a,c,d){var f=this,h=d.find("input:first"),l=this.definition.inputs[c]!==j?this.definition.inputs[c]:
{},g=l.operators!==j?l.operators:[];e.each(a.operators,function(a,c){if(0<g.length&&-1==e.inArray(c,g))return!0;var d=f.createOperatorOptionView(c);e("html").is(".msie")&&d.html("&nbsp;");b.append(d)});b.children().bind("click touchstart",function(a){a.preventDefault();a.target.blur();var g=e(this);a=g.data("name");b.children().removeClass("selected");g.addClass("selected");"exists"==a?d.fadeOut(f.config.animate):d.is(":visible")||d.fadeIn(f.config.animate);"number"==c&&(g=0<=e.inArray(a,l.arrayOperators)?
"enable":"disable",h.jcsdlTagInput(g));f.trigger("operatorSelect",[a])});1==b.children().length?(b.children().eq(0).click(),b.children().hide()):(a=b.find(".operator-"+(a.operator!==j?a.operator:l.operator)),0<a.length?a.click():b.find(".operator:not(.operator-exists):first").click());return b},createTextOperatorsSelectView:function(b,a,c,d){var f=this;e(".jcsdl-dropdown").remove();b.addClass("text");var h=d.find("input.orig"),l=this.getTemplate("textOperatorsSelect"),g=this.getTemplate("textOperatorsDropdown"),
m=this.definition.inputs[c]!==j?this.definition.inputs[c]:{},k=m.operators!==j?m.operators:[];e.each(a.operators,function(a,b){if(0<k.length&&-1==e.inArray(b,k))return!0;var c=f.createTextOperatorOptionView(b);g.append(c)});b.html(l).insertAfter(d);g.hide().appendTo("body");if(a.cs){var q=this.getTemplate("caseSensitivity");q.bind("click touchstart",function(a){a.preventDefault();a.target.blur();e(this).toggleClass("selected");f.trigger("caseSensitivityChange",[e(this).is(".selected")])}).tipsy({gravity:"s",
offset:9});q.prependTo(b);l.addClass("has-cs")}l.click(function(a){a.preventDefault();a.target.blur();a.stopPropagation();l.hasClass("active")?(g.slideUp(f.config.animate),l.removeClass("active")):(g.css({top:l.offset().top+l.outerHeight()-1,left:l.offset().left,width:l.outerWidth()-2}).slideDown(f.config.animate),l.addClass("active"),e("body").bind("click.jcsdldropdown",function(a){a=e(a.target).closest(".jcsdl-dropdown");if(null==a||!a.is(".jcsdl-dropdown"))l.removeClass("active"),g.slideUp(f.config.animate),
e("body").unbind("click.jcsdldropdown")}))});g.find(".jcsdl-dropdown-details-trigger, .jcsdl-operator-help").click(function(a){a.preventDefault();a.target.blur();a.stopPropagation();a=e(this).closest(".jcsdl-dropdown-option");var b=a.data("name"),c=e.jcsdlPopup({title:a.data("title")});f.jsonpCache.operators[b]===j?e.ajax({url:a.data("jsonp"),type:"GET",async:!1,jsonpCallback:"jcsdlJSONP",contentType:"application/json",dataType:"jsonp",success:function(a){!a||!a.html||!e.trim(a.html)?(c.setContent("<p>No documentation available.</p>"),
c.reposition()):(c.setContent(a.html),c.reposition(),f.jsonpCache.operators[b]=a.html)}}):(c.setContent(f.jsonpCache.operators[b]),c.reposition())});g.find(".jcsdl-dropdown-option").click(function(b){b.preventDefault();b.target.blur();l.removeClass("active");g.hide();b=e(this).data("name");var c=f.definition.operators[b];l.data("operator",b).find(".jcsdl-operator-label").html(c.label);"exists"==b?(d.fadeOut(f.config.animate),a.cs&&q.fadeOut(f.config.animate)):d.is(":visible")||(d.fadeIn(f.config.animate),
a.cs&&q.fadeIn(f.config.animate));"contains_near"==b?(d.find(".jcsdl-containsnear-distance").fadeIn(),d.find(".jcsdl-tag-field input").bind("keydown.nospace",function(a){32==a.which&&a.preventDefault()}),h.jcsdlTagInput("update",!0),h.data("split",!0)):(d.find(".jcsdl-containsnear-distance").hide(),d.find(".jcsdl-tag-field input").unbind("keydown.nospace"),h.data("split")&&(h.jcsdlTagInput("update"),h.data("split",!1)));c=0<=e.inArray(b,m.arrayOperators)?"enable":"disable";h.jcsdlTagInput(c);"enable"==
c&&h.jcsdlTagInput("update");c=0<=e.inArray(b,["regex_partial","regex_exact"])?"enable":"disable";h.jcsdlRegExTester(c);"enable"==c&&(h.jcsdlRegExTester("set"+("regex_partial"==b?"Partial":"Exact")),h.jcsdlRegExTester("test"));f.trigger("operatorSelect",[b])});e(window).resize(function(){l.hasClass("active")&&g.css({top:l.offset().top+l.outerHeight()-1,left:l.offset().left,width:l.outerWidth()-2})});1==g.find(".jcsdl-dropdown-option").length?g.find(".jcsdl-dropdown-option:first").click():(c=g.find(".jcsdl-dropdown-option.operator-"+
(a.operator!==j?a.operator:m.operator)),0<c.length?c.click():g.find(".jcsdl-dropdown-option:not(.operator-exists):first").click());return b},createOperatorOptionView:function(b){var a=this.definition.operators[b];return a===j?e():this.getTemplate("operatorOption").data("name",b).addClass("icon-"+b+" operator-"+b).attr("title",a.description).html(a.label).tipsy({gravity:"s",offset:9})},createTextOperatorOptionView:function(b){var a=this.definition.operators[b];if(a===j)return e();var c=this.getTemplate("textOperatorOption").data("name",
b).data("title",a.label).addClass("operator-"+b);c.find(".jcsdl-icon").addClass("icon-"+b+" operator-"+b+" selected");c.find(".jcsdl-operator-label span").html(a.label);c.find(".jcsdl-operator-desc").html(a.description);a.jsonp?c.data("jsonp",a.jsonp):(c.find(".jcsdl-dropdown-details-trigger").remove(),c.find(".jcsdl-operator-help").remove());return c},setValueForField:function(b,a,c,d){var f=this,h=b.data("inputType");if(this.inputs[h]===j)return!1;if(0<=e.inArray(h,["geo_box","geo_radius","geo_polygon"])){var l=
jcsdlMapsCurrentGui,g=jcsdlMapsCurrentCallback,m=jcsdlMapsCurrentCallbackArgs;jcsdlMapsCurrentCallback=function(){l&&g&&setTimeout(function(){g.apply(l.inputs,m);f.inputs.exec(h,"setValue",[b,a,c,d])},l.config.animate+10)}}else this.inputs.exec(h,"setValue",[b,a,c,d]);return!0},getValueFromField:function(b,a,c){var d=b.data("inputType");return this.inputs[d]===j?"":this.inputs.exec(d,"getValue",[b,a,c])},getView:function(){return this.$view},getFilterInfo:function(){var b=this.getFieldInfoAtCurrentPath(),
a="text"==b.input?this.$view.find(".jcsdl-filter-value-input-operators .jcsdl-operators-select").data("operator"):this.$view.find(".jcsdl-filter-value-input-operators .operator.selected").data("name");if(a===j)this.showError("You need to select an operator!");else{var c="";if("exists"!==a&&(c=this.$view.find(".jcsdl-filter-value-input-field"),c=this.getValueFromField(c,b,a),0==c.length)){this.showError("You need to specify a value!");return}return{target:this.target,path:this.path,operator:a,value:c,
additional:{cs:"exists"!==a&&b.cs&&0<this.$view.find(".jcsdl-operator-cs.selected").length?!0:!1}}}},getFieldInfoAtCurrentPath:function(b){var a=this.path.slice(0);"string"==typeof b&&a.push(b);return this.parser.getFieldInfo(this.target,a)},getCurrentPath:function(){return this.target+(0<this.path.length?".":"")+this.path.join(".").replace(/-/g,".")}}});JCSDL.Loader.addComponent(function(e,j){this.GUILogic=function(b,a){var c=this;this.gui=b;this.config=b.config;this.parser=b.parser;this.getTemplate=b.getTemplate;this.$logicOptions=b.$mainView.find(".jcsdl-filters-logic .jcsdl-filters-logic-option:not(.jcsdl-advanced-option)");this.$advancedOptions=b.$mainView.find(".jcsdl-filters-logic .jcsdl-filters-logic-option.jcsdl-advanced-option");this.$insertBracketsButton=this.$advancedOptions.filter(".jcsdl-advanced-add-brackets");this.$insertNotOperatorButton=
...
("object"==typeof this.config.saveButton||"string"==typeof this.config.saveButton))this.$saveButton.remove(),this.$saveButton=e(this.config.saveButton);if(this.config.cancelButton&&("object"==typeof this.config.cancelButton||"string"==typeof this.config.cancelButton))this.$cancelButton.remove(),this.$cancelButton=e(this.config.cancelButton);this.config.cancelButton||this.$cancelButton.hide();var a=/(msie) ([\w.]+)/.exec(navigator.userAgent.toLowerCase())||[];a[1]!==j&&"msie"===a[1]&&(this.$container.addClass("msie"),
e("html").addClass("msie"));this.logic=new JCSDL.GUILogic(this,JCSDL.GUILogic.prototype.AND);var a=this.$mainView.find(".jcsdl-elements-sprite:first").css("background-image").replace(/url\((\'|\")?/,"").replace(/(\'|\")?\)$/,""),c=document.createElement("a");c.href=a;a=c.pathname.split("/");a.pop();a.pop();this.assetsUrl=a.join("/")+"/";this.config.mapsMarker||(this.config.mapsMarker=this.assetsUrl+"img/maps-marker.png");this.config.zeroClipboard||(this.config.zeroClipboard=this.assetsUrl+"swf/ZeroClipboard.swf");
...
b.showFilterEditor()});this.$saveButton.unbind(".jcsdl").bind("click.jcsdl touchstart.jcsdl",function(a){a.preventDefault();a.target.blur();try{var c=b.returnJCSDL();b.trigger("save",[c])}catch(e){b.showError(e),b.trigger("saveError",[e instanceof JCSDL.LogicValidationException?e.message:e])}});this.$previewButton.unbind(".jcsdl").bind("click.jcsdl touchstart.jcsdl",function(a){a.preventDefault();a.target.blur();try{var c=b.returnJCSDL().split("\n"),h=[];e.each(c,function(a,b){"//"!==b.substr(0,2)&&
h.push(b)});var j=h.join("\n"),g="jcsdl-copy-to-clipboard-"+Math.floor(1E4*Math.random());e.jcsdlPopup({title:"CSDL Preview",content:["<p>"+b.highlightCSDL(j)+"</p>",'<a href="#" id="'+g+'" class="jcsdl-btn jcsdl-copy-to-clipboard"><i class="jcsdl-picto jcsdl-elements-sprite"></i></a>'].join("")});var m=e("#"+g).zclip({path:b.config.zeroClipboard,copy:j,afterCopy:function(){clearTimeout(void 0);m.addClass("copied");setTimeout(function(){m.removeClass("copied")},3E3)}})}catch(k){b.showError(k)}return!1});
this.$cancelButton.unbind(".jcsdl").bind("click.jcsdl touchstart.jcsdl",function(a){a.preventDefault();a.target.blur();b.trigger("cancel")})},reset:function(){this.init()},loadJCSDL:function(b){this.init();var a=this;try{var c=this.parser.parseJCSDL(b);this.filters=c.filters;2<=this.filters.length&&this.logic.toggleButtons();this.logic.setLogic(c.logic,!0)}catch(d){return b=d instanceof JCSDL.ValidationException?d.code:b,this.showError(d,b),this.trigger("invalidJCSDL",[b]),!1}e.each(this.filters,
...
console!==j&&console.error(c,arguments);d.animateIntoView(this.config.animate)},clearErrors:function(){this.$mainView.find(".jcsdl-error").remove()},createFilterRow:function(b){var a=this,c=this.getTemplate("filter");c.addClass("filter-"+b.id).data("filterId",b.id).find(".jcsdl-filter-info.id").html(b.id);var d=this.parser.getTargetInfo(b.target),d=this.getTemplate("filterTarget").addClass("selected target-"+b.target).html(d.name);c.find(".target").html(d);var f=[];e.each(b.fieldPath,function(d,e){f.push(e);
var g=a.parser.getFieldInfo(b.target,f),g=a.getTemplate("filterField").addClass("selected icon-"+a.getIconForField(e,g)).html(g.name);c.find(".jcsdl-filter-info.field").append(g)});var d=this.parser.getFieldInfo(b.target,f),h=this.getTemplate("filterOperator").addClass("operator-"+b.operator+" icon-"+b.operator+" selected").attr("title",this.definition.operators[b.operator].description).html(this.definition.operators[b.operator].code.escapeHtml()).tipsy({gravity:"s",offset:9});c.find(".operator").html(h);
if(b.cs){var j=this.getTemplate("filterOperator").addClass("operator-cs icon-cs selected").attr("title","Case Sensitive").html("case sensitive").tipsy({gravity:"s",offset:9});c.find(".jcsdl-filter-info.operator").append(j);j.tipsy("show")}if("exists"!==b.operator){var g=null,j=(g=0<=e.inArray(b.operator,["geo_box","geo_polygon","geo_radius"]))?b.operator:d.input;g&&(this.getTemplate("filterFieldInput").insertAfter(c.find(".jcsdl-filter-info.field")).find(".jcsdl-filter-field-input").addClass("selected icon-"+
