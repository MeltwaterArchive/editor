...
!1)})},getGuiString:function(a){a=a||this.$gui;var b="";a.find(".jcsdl-logic-token").each(function(){b+=""+e(this).data("token")});return b},getGuiTokens:function(a){a=a||this.$gui;var b=[];a.find(".jcsdl-logic-token").each(function(){b.push(e(this).data("token"))});return b},clearGui:function(){this.$gui.width(0).children().remove()},tokensMap:{"(":"bracketOpen",")":"bracketClose","&":"operatorAnd","|":"operatorOr",filter:"filter"},addGuiToken:function(a,b,c){b=b===h?!0:b;c=c===h?!1:c;var d=this,
f="number"==typeof a?"filter":a;if(this.tokensMap[f]===h)throw'Invalid character "'+a+'" in the logic!';var g=this.getTemplate("logicToken_"+this.tokensMap[f]).data("token",a),j=e();"filter"==f&&g.html(a);f=e('<li class="jcsdl-logic-token-wrap" />').html(g).appendTo(this.$gui);this.$gui.width(this.$gui.width()+f.outerWidth(!0));if(")"==a&&c){j=this.getTemplate("logicToken_bracketOpen").data("token","(");a=this.getGuiTokens().reverse();var k=a.length,l=-1,m=null;e.each(a,function(a,b){")"==b?l++:"("==
b&&l--;if(0==l&&0<=e.inArray(b,["&","|"])){if(null!==m&&b!==m)return k=a,!1;m=b}});a=this.$gui.children().eq(a.length-k);a=e('<li class="jcsdl-logic-token-wrap" />').html(j).insertBefore(a);this.$gui.width(this.$gui.width()+a.outerWidth(!0))}this.updateFromGui();this.adjustGui();b&&this.scrollGuiToToken(g);g.click(function(a){a.preventDefault();a.target.blur();"&"==g.data("token")?g.data("token","|").html("OR"):"|"==g.data("token")&&g.data("token","&").html("AND");d.updateFromGui()});g.add(j).find(".jcsdl-delete-token").click(function(a){a.preventDefault();
