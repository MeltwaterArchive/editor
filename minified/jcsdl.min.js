...
e);c=this.filterFromJCSDL(f,e,g);d.push(c)}return{filters:d,logic:a}},filterFromJCSDL:function(a,b,d){a=a.split(",");var c=a.shift().split("."),e=c.shift(),f=this.getFieldInfo(e,c);if(!1===f)return!1;var h=a.shift(),j="",l={cs:!1};"exists"!==h&&(j=a.shift().split("-"),j=this.valueFromCSDL(f,b.substr(j[0],j[1]),h),g.each(a,function(a,b){switch(b){case "cs":l.cs=!0}}));return this.createFilter(e,c,h,j,l,d)},getJCSDLForFilters:function(a,b){var d=this,c={},e=[],f="";g.each(a,function(a,b){var e=d.filterToCSDL(b);
if(!e)return!0;c[b.id+""]=e});if("AND"!=b&&"OR"!=b){var f=this.tokenizeLogicString(b),h="";this.validateLogic(f,a);g.each(f,function(a,b){if("number"===typeof b)if(h.length&&e.push(g.trim(h)),h="",c[b+""]!==k)e.push(c[b+""]);else throw new JCSDL.LogicValidationException("A filter with ID '"+b+"' doesn't exist! You cannot use it in a logical expression",a);else h="&"==b||"|"==b?h+("|"==b?" OR ":" AND "):h+b});h.length&&e.push(g.trim(h));f=e.join("\n")}else g.each(c,function(b,a){e.push(a)}),f=e.join("\n"+
...
b);c=a});var j=[];g.each(h,function(a,b){-1==g.inArray(b,e)&&j.push(b)});if(j.length)throw new JCSDL.LogicValidationException("You haven't used some of the filters in your logical expression (with ID's: \""+j.join('", "')+'"). All filters need to be used!');if(0!==f)throw new JCSDL.LogicValidationException("You have't closed some parentheses!");return!0},error:function(a,b){this.gui.showError.apply(this.gui,arguments)},createFilter:function(a,b,d,c,e,f){e=e||{};return{id:f,target:a,fieldPath:b,operator:d,
value:c,cs:e.cs}},getOperatorCode:function(a){return this.gui.definition.operators[a].code},getTargetInfo:function(a){if(this.gui.definition.targets[a]!==k)return this.gui.definition.targets[a];this.error("Such target does not exist!",a);return!1},getFieldInfo:function(a,b){var d=this,c=this.gui.definition.targets[a];if(c===k)return this.error("Such target does not exist!",a),!1;g.each(b,function(e,f){if(c.fields!==k)if(c.fields[f]!==k)c=c.fields[f];else return d.error("Invalid path to a field (#1)!",
...
50)},50));b.$guiLeft.is(":visible")&&c.offset.left<=b.$guiLeft.offset().left+c.helper.width()/2&&(d=setInterval(function(){b.scrollGui("left",50)},50));var e=c.helper.find(".jcsdl-logic-token");e.is(".filter")&&b.gui.highlightFilter(e.html())},update:function(){clearInterval(d);setTimeout(function(){b.updateFromGui();var a=b.getGuiString();b.trigger("graphicalLogicTokenMove",[a]);b.trigger("graphicalLogicChange",[a]);b.trigger("advancedLogicChange",[a])},100)}});c.data("sortable").floating=!0;c.data("uiSortable").floating=
!0;this.$insertBracketsButton.click(function(a){a.preventDefault();a.target.blur();if(e(this).is(".disabled"))return!1;b.addGuiToken(")",!0,!0);b.updateFromGui();a=b.getGuiString();b.trigger("parenthesisAdd",[a]);b.trigger("graphicalLogicChange",[a]);b.trigger("advancedLogicChange",[a])});this.$guiRight.add(this.$guiLeft).bind("mousedown touchstart",function(){var a=e(this).is(".right")?"right":"left";d=setInterval(function(){b.scrollGui(a,50)},50)}).bind("mouseup mouseleave touchend",function(){clearInterval(d)}).click(function(a){a.preventDefault();
a.target.blur()});e(window).resize(function(){b.adjustGui()});return a},updateFromGui:function(){var a=this.getGuiTokens();try{this.validate(a)&&(this.clearErrors(),this.logicString=a.join(""))}catch(b){this.showError(b,!0)}},setGuiString:function(a){a=a||this.logicString;var b=this;a="string"===typeof a||"number"===typeof a?this.tokenize(a):a;this.clearGui();e.each(a,function(a,d){b.addGuiToken(d,!1)})},getGuiString:function(a){a=a||this.$gui;var b="";a.find(".jcsdl-logic-token").each(function(){b+=
""+e(this).data("token")});return b},getGuiTokens:function(a){a=a||this.$gui;var b=[];a.find(".jcsdl-logic-token").each(function(){b.push(e(this).data("token"))});return b},clearGui:function(){this.$gui.width(0).children().remove()},tokensMap:{"(":"bracketOpen",")":"bracketClose","&":"operatorAnd","|":"operatorOr",filter:"filter"},addGuiToken:function(a,b,c){b=b===h?!0:b;c=c===h?!1:c;var d=this,f="number"==typeof a?"filter":a;if(this.tokensMap[f]===h)throw'Invalid character "'+a+'" in the logic!';
var g=this.getTemplate("logicToken_"+this.tokensMap[f]).data("token",a),j=e();"filter"==f&&g.html(a);f=e('<li class="jcsdl-logic-token-wrap" />').html(g).appendTo(this.$gui);this.$gui.width(this.$gui.width()+f.outerWidth(!0));if(")"==a&&c){j=this.getTemplate("logicToken_bracketOpen").data("token","(");a=this.getGuiTokens().reverse();var k=a.length,l=-1,m=null;e.each(a,function(a,b){")"==b?l++:"("==b&&l--;if(0==l&&0<=e.inArray(b,["&","|"])){if(null!==m&&b!==m)return k=a,!1;m=b}});a=this.$gui.children().eq(a.length-
k);a=e('<li class="jcsdl-logic-token-wrap" />').html(j).insertBefore(a);this.$gui.width(this.$gui.width()+a.outerWidth(!0))}this.updateFromGui();this.adjustGui();b&&this.scrollGuiToToken(g);g.click(function(a){a.preventDefault();a.target.blur();"&"==g.data("token")?(g.data("token","|").html("OR"),d.trigger("logicOperatorSwitch",["OR"]),a=d.getGuiString(),d.trigger("graphicalLogicChange",[a]),d.trigger("advancedLogicChange",[a])):"|"==g.data("token")&&(g.data("token","&").html("AND"),d.trigger("logicOperatorSwitch",
["AND"]),a=d.getGuiString(),d.trigger("graphicalLogicChange",[a]),d.trigger("advancedLogicChange",[a]));d.updateFromGui()});g.add(j).find(".jcsdl-delete-token").click(function(a){a.preventDefault();a.target.blur();a.stopPropagation(!0);d.deleteGuiToken(g);a=d.getGuiString();d.trigger("parenthesisDelete",[a]);d.trigger("graphicalLogicChange",[a]);d.trigger("advancedLogicChange",[a])});g.filter(".filter").hover(function(){d.gui.highlightFilter(e(this).data("token"))},function(){d.gui.highlightFilter(e(this).data("token"),
!1)})},deleteGuiFilterToken:function(a){a=parseInt(a);var b=e(),c=e();this.$gui.children().each(function(){var c=e(this);if(parseInt(c.find(".jcsdl-logic-token").data("token"))===a)return b=c,!1});if(!b.length)return!1;for(;"("==b.prev().find(".jcsdl-logic-token").data("token")&&")"==b.next().find(".jcsdl-logic-token").data("token");)b.prev().remove(),b.next().remove();b.prev().find(".jcsdl-logic-token.operator").length?c=b.prev():b.next().find(".jcsdl-logic-token.operator").length&&(c=b.next());
b.remove();c.remove();this.updateFromGui();c=self.getGuiString();self.trigger("graphicalLogicChange",[c]);self.trigger("advancedLogicChange",[c])},deleteGuiToken:function(a){var b=a.is(".jcsdl-logic-token")?a.closest("li.jcsdl-logic-token-wrap"):a;if(!b.is("li.jcsdl-logic-token-wrap"))return!1;a=b.find(".jcsdl-logic-token");if(a.is(".operator"))return!1;if(a.is(".filter")){var c=a.data("token"),d=!1;e.each(this.gui.filters,function(a,b){if(parseInt(b.id)==parseInt(c))return d=!0,!1});if(d)return!1}else if(a.is(".bracket")){var f=
this.getGuiTokens(),g=this.$gui.children().index(b),j=null,k=0,l=0,m=")",h="(",n=")";")"==a.data("token")&&(f=f.reverse(),g=f.length-1-g,m="(",h=")",n="(");e.each(f,function(a,b){if(a==g)return l=k,!0;if(null!==l&&k==l&&a>g&&b==m)return j="("==m?f.length-1-a:a,!1;b==h?k++:b==n&&k--});null!==j&&(a=this.$gui.children().filter(".jcsdl-logic-token-wrap").eq(j),this.$gui.width(this.$gui.width()-a.outerWidth(!0)),a.remove())}this.$gui.width(this.$gui.width()-b.outerWidth(!0));b.remove();this.updateFromGui();
this.adjustGui()},_adjustGuiTimeout:null,adjustGui:function(){var a=this,b=0;this.$gui.children().each(function(){b+=parseInt(e(this).outerWidth(!0))});this.$gui.width(b);clearTimeout(this._adjustGuiTimeout);this._adjustGuiTimeout=setTimeout(function(){var b=a.$gui.width(),d=a.$guiContainer.width();b>d?a.$guiRight.removeClass("off"):(a.$guiLeft.add(a.$guiRight).addClass("off"),a.scrollGuiToStart())},50)},scrollGui:function(a,b){b=b==h?50:b;var c=-1*Math.max(0,this.$gui.width()-this.$guiContainer.width()),
d=0,f=this,d="number"===typeof a?a:parseInt(this.$gui.css("left"))+("right"==a?-30:30),d=Math.max(Math.min(0,d),c);this.$gui.stop(!0,!0).animate({left:d},b,function(){f.$guiLeft[0==d?"addClass":"removeClass"]("off");f.$guiRight[d==c?"addClass":"removeClass"]("off")})},scrollGuiToStart:function(){this.scrollGui(0)},scrollGuiToEnd:function(){this.scrollGuiToToken(this.$gui.children().last())},scrollGuiToToken:function(a){a=a.is(".jcsdl-logic-token")?a.closest("li.jcsdl-logic-token-wrap"):a;if(!a.is("li.jcsdl-logic-token-wrap"))return!1;
a=-1*a.position().left;parseInt(this.$gui.css("left"));this.scrollGui(a+140)},initManualInput:function(a){var b=this,c=null;a.keyup(function(){clearTimeout(c);c=setTimeout(function(){var c=a.val();b.logicString!==c&&(b.trigger("manualLogicChange",[c]),b.trigger("advancedLogicChange",[c]));try{b.validate(c)&&(b.clearErrors(),b.logicString=c)}catch(f){b.showError(f,!0)}},500)});a.keydown(function(a){var b=a.which,c=a.shiftKey;8==b||(37<=b&&40>=b||46==b||48<=b&&57>=b&&!c||96<=b&&105>=b||219==b||57==
b&&c||221==b||48==b&&c||32==b||55==b&&c||220==b&&c)||a.preventDefault()});a.keypress(function(a){13==a.which&&a.preventDefault()});return a},setManualString:function(a){a=a||this.logicString;a="string"===typeof a||"number"===typeof a?this.tokenize(a):a;this.$manualInput.val(a.join(" "))},getManualString:function(){return this.$manualInput.val()},setLogic:function(a,b){b=b||!1;var c=this.logic;this.logic=-1<e.inArray(a,[this.AND,this.OR])?a:this.ADVANCED;var d="";if(this.logic==this.ADVANCED)d=a;else{var f=
[];e.each(this.gui.filters,function(a,b){f.push(b.id)});d=f.join(this.logic==this.OR?"|":"&")}this.setLogicString(d);this.didSetLogic(c,b)},getLogic:function(){return this.logic},setLogicString:function(a){this.logicString=""+a;this.setGuiString(a);this.setManualString(a)},appendFilterToLogicString:function(a,b){b=b||this.logic;b=b==this.OR?"|":"&";var c=this.logicString.length?this.logicString+b+a:a;this.trigger("manualLogicChange",[c]);this.trigger("graphicalLogicChange",[c]);this.trigger("advancedLogicChange",
[c]);this.setLogicString(c)},getLogicString:function(){return this.logic==this.ADVANCED?this.logicString:this.logic},didSetLogic:function(a,b){a=a||"";b=b||!1;this.$logicOptions.removeClass("active").filter('[data-logic="'+this.logic+'"]').addClass("active");this.logic==this.ADVANCED?(this.showEditor(),this.$logicOptions.not(".jcsdl-advanced").each(function(){var a=e(this);a.html(a.data("textShort"))}),this.$advancedOptions.fadeIn(this.config.animate)):(this.clearErrors(),this.hideEditor(),this.$logicOptions.not(".jcsdl-advanced").each(function(){var a=
e(this);a.html(a.data("textLong"))}),this.$advancedOptions.hide());!b&&this.logic!==a&&this.trigger("logicChange",[this.logic])}}});;
...
this.getTemplate("editor");this.$filtersList=this.$mainView.find(".jcsdl-filters-list");this.$container=this.getTemplate("container");this.$container.html(this.$mainView);this.$el.html(this.$container);this.$saveButton=this.$mainView.find(".jcsdl-editor-save");this.$cancelButton=this.$mainView.find(".jcsdl-editor-cancel");if(this.config.saveButton&&("object"==typeof this.config.saveButton||"string"==typeof this.config.saveButton))this.$saveButton.remove(),this.$saveButton=e(this.config.saveButton);
if(this.config.cancelButton&&("object"==typeof this.config.cancelButton||"string"==typeof this.config.cancelButton))this.$cancelButton.remove(),this.$cancelButton=e(this.config.cancelButton);this.config.cancelButton||this.$cancelButton.hide();e.browser.msie&&this.$container.addClass("msie");this.logic=new JCSDL.GUILogic(this,JCSDL.GUILogic.prototype.AND);this.$mainView.find(".jcsdl-mainview-mode .jcsdl-mainview-mode-option").bind("click touchstart",function(b){b.preventDefault();b.target.blur();b=
e(this);var c=b.attr("data-mode");a.$filtersList.removeClass("expanded collapsed");a.$filtersList.addClass(c);a.$mainView.find(".jcsdl-mainview-mode .jcsdl-mainview-mode-option").removeClass("active");b.addClass("active");a.trigger("viewModeChange",[c])});this.$mainView.find(".jcsdl-add-filter").bind("click touchstart",function(b){b.preventDefault();b.target.blur();a.showFilterEditor()});this.$saveButton.unbind(".jcsdl").bind("click.jcsdl touchstart.jcsdl",function(b){b.preventDefault();b.target.blur();
try{var c=a.returnJCSDL();a.trigger("save",[c])}catch(d){a.showError(d),a.trigger("saveError",[d instanceof JCSDL.LogicValidationException?d.message:d])}});this.$cancelButton.unbind(".jcsdl").bind("click.jcsdl touchstart.jcsdl",function(b){b.preventDefault();b.target.blur();a.trigger("cancel")})},reset:function(){this.init()},loadJCSDL:function(a){this.init();var b=this;try{var c=this.parser.parseJCSDL(a);this.filters=c.filters;2<=this.filters.length&&this.logic.toggleButtons();this.logic.setLogic(c.logic,
!0)}catch(d){return a=d instanceof JCSDL.ValidationException?d.code:a,this.showError(d,a),this.trigger("invalidJCSDL",[a]),!1}e.each(this.filters,function(a,c){b.createFilterRow(c).appendTo(b.$filtersList)});this.filters.length&&this.$mainView.find(".jcsdl-footer").show();this.config.cancelButton&&this.$cancelButton.show()},returnJCSDL:function(){return this.parser.getJCSDLForFilters(this.filters,this.logic.getLogicString())},adjust:function(){this.editor&&this.editor.adjust()},hide:function(){this.$container.hide()},
show:function(){this.$container.show()},trigger:function(a,b){b=b||[];this.$el.trigger(a.toLowerCase(),b);return"function"==typeof this.config[a]?this.config[a].apply(this,b):null},showFilterEditor:function(a,b){this.$mainView.hide();this.editor=new JCSDL.GUIFilterEditor(this,this.$container,a);if(a!==f&&a)this.currentFilterIndex=b,this.trigger("filterEdit",[a]);else{this.trigger("filterNew");var c=this.filters[this.filters.length-1];c!==f&&this.editor.getView().find(".jcsdl-filter-target .target-"+
c.target+":visible").trigger("jcsdlclick")}},hideFilterEditor:function(a){a="function"==typeof a?a:e.noop;var b=this;this.currentFilterIndex=null;this.editor.getView().fadeOut(this.config.animate,function(){e(this).remove();b.$mainView.show();b.editor=null;a.apply(b,[])})},didSubmitFilter:function(a){var b=this,c=e.noop,d=null!==this.currentFilterIndex?this.filters[this.currentFilterIndex].id:this.createFilterId(),g=this.parser.createFilter(a.target,a.path,a.operator,a.value,a.additional,d);a=this.createFilterRow(g);
null!==this.currentFilterIndex?(this.$filtersList.find(".jcsdl-filter.filter-"+g.id).replaceWith(a),this.filters[this.currentFilterIndex]=g):(a.appendTo(this.$filtersList),this.filters.push(g),c=function(){b.logic.appendFilterToLogicString(g.id)});this.filters.length&&this.$mainView.find(".jcsdl-footer").fadeIn(this.config.animate);2<=this.filters.length&&this.logic.toggleButtons();this.hideFilterEditor(c);this.trigger("filterSave",[g])},deleteFilter:function(a){var b=this.filters[a];this.trigger("filterDelete",
[b]);this.$filtersList.find(".jcsdl-filter.filter-"+b.id).remove();this.filters.splice(a,1);this.logic.deleteGuiFilterToken(b.id);this.filters.length||this.$mainView.find(".jcsdl-footer").fadeOut(this.config.animate);2>this.filters.length&&this.logic.toggleButtons(!1)},showError:function(a,b){if(a instanceof TypeError)throw a;this.clearErrors();if(a instanceof JCSDL.LogicValidationException)return this.logic.showError(a);var c=a instanceof JCSDL.ValidationException?a.message:a,d=this.getTemplate("error");
d.find("span").html(c);d.insertBefore(this.$mainView.find(".jcsdl-footer"));d.hide().fadeIn();console!==f&&console.error(c,arguments);d.animateIntoView(this.config.animate)},clearErrors:function(){this.$mainView.find(".jcsdl-error").remove()},createFilterRow:function(a){var b=this,c=this.getTemplate("filter");c.addClass("filter-"+a.id).data("filterId",a.id).find(".jcsdl-filter-info.id").html(a.id);var d=this.parser.getTargetInfo(a.target),d=this.getTemplate("filterTarget").addClass("selected target-"+
a.target).html(d.name);c.find(".target").html(d);var g=[];e.each(a.fieldPath,function(d,e){g.push(e);var f=b.parser.getFieldInfo(a.target,g),f=b.getTemplate("filterField").addClass("selected icon-"+b.getIconForField(e,f)).html(f.name);c.find(".jcsdl-filter-info.field").append(f)});var d=this.parser.getFieldInfo(a.target,g),f=this.getTemplate("filterOperator").addClass("operator-"+a.operator+" icon-"+a.operator+" selected").attr("title",this.definition.operators[a.operator].description).html(this.definition.operators[a.operator].code.escapeHtml()).tipsy({gravity:"s",
offset:9});c.find(".operator").html(f);if(a.cs){var h=this.getTemplate("filterOperator").addClass("operator-cs icon-cs selected").attr("title","Case Sensitive").html("case sensitive").tipsy({gravity:"s",offset:9});c.find(".jcsdl-filter-info.operator").append(h);h.tipsy("show")}if("exists"!==a.operator){var j=null,h=(j=0<=e.inArray(a.operator,["geo_box","geo_polygon","geo_radius"]))?a.operator:d.input;j&&(this.getTemplate("filterFieldInput").insertAfter(c.find(".jcsdl-filter-info.field")).find(".jcsdl-filter-field-input").addClass("selected icon-"+
a.operator),f.remove());j=this.inputs[h]&&"function"==typeof this.inputs[h].displayValue?this.inputs.exec(h,"displayValue",[d,a.value,a]):this.getTemplate("filterValue").html(a.value.truncate(50).escapeHtml());c.find(".value").addClass("input-"+h).html(j);c.find(".operator").addClass("input-"+h)}else c.find(".value").remove();c.data("filter",a);c.find(".edit").bind("click touchstart",function(d){d.preventDefault();d.target.blur();b.showFilterEditor(a,b.getFilterIndexByElement(c))});c.find(".delete").bind("click touchstart",
function(a){a.preventDefault();a.target.blur();b.deleteFilter(b.getFilterIndexByElement(c))});return c},createFilterId:function(){var a=[],b=1;for(e.each(this.filters,function(b,d){a.push(parseInt(d.id))});0<=e.inArray(b,a);)b++;return b},highlightFilter:function(a,b){b=b===f?!0:!1;this.$filtersList.find(".filter-"+a)[b?"addClass":"removeClass"]("on")},getTemplate:function(a){return JCSDL.GUITemplates[a]!==f?JCSDL.GUITemplates[a].clone():e()},getIconForField:function(a,b){return b.icon!==f?b.icon:
a},getFilterIndexByElement:function(a){return this.$filtersList.find(".jcsdl-filter").index(a)}};e.fn.jcsdlGui=function(a){function b(b){var c=b.data("jcsdlGui");c||(c=new JCSDL.GUI(b,a),b.data("jcsdlGui",c));return c}if("string"==typeof a){if(0<=e.inArray(a,["loadJCSDL","adjust"])){var c=[];e.each(arguments,function(a,b){if(0==a)return!0;c.push(b)});this.each(function(){var d=b(e(this));d[a].apply(d,c)})}return this}this.each(function(){b(e(this))});return this};JCSDLGui=function(a,b){return new JCSDL.GUI(a,
b)}});;
