...
var url=self.config.targetHelpUrl.apply(self,[cacheName.replace(/-/g,'.'),info]);if(typeof(self.jsonpCache.targets[cacheName])=='undefined'){$.ajax({url:url,type:'GET',async:false,jsonpCallback:'jcsdlJSONP',contentType:'application/json',dataType:'jsonp',success:function(data){if(!data||!data.html||!data.html.trim()){popup.setContent('<p>No documentation available.</p>');popup.reposition();return;}
