...
this.$gui.is(":visible")&&0<=a.index&&(c=this.$gui.children().not(".jcsdl-token-ph").eq(a.index).find(".jcsdl-logic-token").addClass("jcsdl-logic-error"),this.scrollGuiToToken(c));d.animateIntoView(this.config.animate)},trigger:function(a,b){this.gui.trigger(a,b)},clearErrors:function(){this.gui.clearErrors();this.$gui.find(".jcsdl-logic-error").removeClass("jcsdl-logic-error")},initGui:function(a){var b=this,c=a.find(".jcsdl-advanced-gui"),d=null;this.$guiLeft=a.find(".jcsdl-advanced-gui-arrow.left");
this.$guiRight=a.find(".jcsdl-advanced-gui-arrow.right");c.sortable({cursor:"move",placeholder:"jcsdl-token-ph",containment:a,appendTo:a,helper:"clone",tolerance:"intersect",start:function(a,b){b.item.find(".jcsdl-logic-token").is(".bracket")&&c.find(".jcsdl-token-ph").addClass("bracket")},stop:function(a,c){var d=c.item.find(".jcsdl-logic-token");d.is(".filter")&&b.gui.highlightFilter(d.data("token"),!1)},sort:function(a,c){clearInterval(d);b.$guiRight.is(":visible")&&c.offset.left+c.helper.width()/
2>=b.$guiRight.offset().left&&(d=setInterval(function(){b.scrollGui("right",50)},50));b.$guiLeft.is(":visible")&&c.offset.left<=b.$guiLeft.offset().left+c.helper.width()/2&&(d=setInterval(function(){b.scrollGui("left",50)},50));var e=c.helper.find(".jcsdl-logic-token");e.is(".filter")&&b.gui.highlightFilter(e.html())},update:function(){clearInterval(d);setTimeout(function(){b.updateFromGui();var a=b.getGuiString();b.trigger("graphicalLogicTokenMove",[a]);b.trigger("graphicalLogicChange",[a]);b.trigger("advancedLogicChange",
[a])},100)}});(c.data("sortable")||{}).floating=!0;(c.data("uiSortable")||{}).floating=!0;this.$insertBracketsButton.click(function(a){a.preventDefault();a.target.blur();if(e(this).is(".disabled"))return!1;b.addGuiToken(")",!0,!0);b.updateFromGui();a=b.getGuiString();b.trigger("parenthesisAdd",[a]);b.trigger("graphicalLogicChange",[a]);b.trigger("advancedLogicChange",[a])});this.$guiRight.add(this.$guiLeft).bind("mousedown touchstart",function(){var a=e(this).is(".right")?"right":"left";d=setInterval(function(){b.scrollGui(a,
...
b&&l--;if(0==l&&0<=e.inArray(b,["&","|"])){if(null!==m&&b!==m)return k=a,!1;m=b}});a=this.$gui.children().eq(a.length-k);a=e('<li class="jcsdl-logic-token-wrap" />').html(j).insertBefore(a);this.$gui.width(this.$gui.width()+a.outerWidth(!0))}this.updateFromGui();this.adjustGui();b&&this.scrollGuiToToken(g);g.click(function(a){a.preventDefault();a.target.blur();"&"==g.data("token")?(g.data("token","|").html("OR"),d.trigger("logicOperatorSwitch",["OR"]),a=d.getGuiString(),d.trigger("graphicalLogicChange",
[a]),d.trigger("advancedLogicChange",[a])):"|"==g.data("token")&&(g.data("token","&").html("AND"),d.trigger("logicOperatorSwitch",["AND"]),a=d.getGuiString(),d.trigger("graphicalLogicChange",[a]),d.trigger("advancedLogicChange",[a]));d.updateFromGui()});g.add(j).find(".jcsdl-delete-token").click(function(a){a.preventDefault();a.target.blur();a.stopPropagation(!0);d.deleteGuiToken(g);a=d.getGuiString();d.trigger("parenthesisDelete",[a]);d.trigger("graphicalLogicChange",[a]);d.trigger("advancedLogicChange",
[a])});g.filter(".filter").hover(function(){d.gui.highlightFilter(e(this).data("token"))},function(){d.gui.highlightFilter(e(this).data("token"),!1)})},deleteGuiFilterToken:function(a){a=parseInt(a);var b=e(),c=e();this.$gui.children().each(function(){var c=e(this);if(parseInt(c.find(".jcsdl-logic-token").data("token"))===a)return b=c,!1});if(!b.length)return!1;for(;"("==b.prev().find(".jcsdl-logic-token").data("token")&&")"==b.next().find(".jcsdl-logic-token").data("token");)b.prev().remove(),b.next().remove();
...
a.data("token"),d=!1;e.each(this.gui.filters,function(a,b){if(parseInt(b.id)==parseInt(c))return d=!0,!1});if(d)return!1}else if(a.is(".bracket")){var f=this.getGuiTokens(),g=this.$gui.children().index(b),j=null,k=0,l=0,m=")",h="(",n=")";")"==a.data("token")&&(f=f.reverse(),g=f.length-1-g,m="(",h=")",n="(");e.each(f,function(a,b){if(a==g)return l=k,!0;if(null!==l&&k==l&&a>g&&b==m)return j="("==m?f.length-1-a:a,!1;b==h?k++:b==n&&k--});null!==j&&(a=this.$gui.children().filter(".jcsdl-logic-token-wrap").eq(j),
...
[c]));try{b.validate(c)&&(b.clearErrors(),b.logicString=c)}catch(f){b.showError(f,!0)}},500)});a.keydown(function(a){var b=a.which,c=a.shiftKey;8==b||(37<=b&&40>=b||46==b||48<=b&&57>=b&&!c||96<=b&&105>=b||219==b||57==b&&c||221==b||48==b&&c||32==b||55==b&&c||220==b&&c)||a.preventDefault()});a.keypress(function(a){13==a.which&&a.preventDefault()});return a},setManualString:function(a){a=a||this.logicString;a="string"===typeof a||"number"===typeof a?this.tokenize(a):a;this.$manualInput.val(a.join(" "))},
getManualString:function(){return this.$manualInput.val()},setLogic:function(a,b){b=b||!1;var c=this.logic;this.logic=-1<e.inArray(a,[this.AND,this.OR])?a:this.ADVANCED;var d="";if(this.logic==this.ADVANCED)d=a;else{var f=[];e.each(this.gui.filters,function(a,b){f.push(b.id)});d=f.join(this.logic==this.OR?"|":"&")}this.setLogicString(d);this.didSetLogic(c,b)},getLogic:function(){return this.logic},setLogicString:function(a){this.logicString=""+a;this.setGuiString(a);this.setManualString(a)},appendFilterToLogicString:function(a,
