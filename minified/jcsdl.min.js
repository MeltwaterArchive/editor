...
var popup=$.jcsdlPopup({title:title});if(cacheName.split('.')[0]=='augmentation'){cacheName=cacheName.substr(13);}
var url=self.config.targetHelpUrl.apply(self,[cacheName.replace(/-/g,'.'),info]);if(typeof(self.jsonpCache.targets[cacheName])=='undefined'){$.ajax({url:url,type:'GET',async:false,jsonpCallback:'jcsdlJSONP',contentType:'application/json',dataType:'jsonp',success:function(data){if(!data||!data.html||!$.trim(data.html)){popup.setContent('<p>No documentation available.</p>');popup.reposition();return;}
