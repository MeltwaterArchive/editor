...
AND:"AND",OR:"OR",tokenize:function(a){a=a||this.logicString;return this.parser.tokenizeLogicString(a)},validate:function(a,b){a=a||this.logicString;b=b||this.gui.filters;return this.parser.validateLogic(a,b)},showEditor:function(){this.$editor.show();this.adjustGui();this.$editor.hide().slideDown(this.config.animate);this.gui.$mainView.addClass("jcsdl-advanced-on")},hideEditor:function(){this.$editor.slideUp(this.config.animate);this.gui.$mainView.removeClass("jcsdl-advanced-on")},toggleButtons:function(a){(a=
a===h?!0:a)||this.setLogic(this.AND);this.$logicOptions[a?"removeClass":"addClass"]("disabled")},showError:function(a,b){b=b||!1;this.clearErrors();var c=a instanceof JCSDL.LogicValidationException?a.message:a,d=this.getTemplate("error");d.find("span").html(c);d.insertAfter(this.$editor);d.show();!b&&console!==h&&console.error(a,arguments);a instanceof JCSDL.LogicValidationException&&this.$gui.is(":visible")&&0<=a.index&&(c=this.$gui.children().not(".jcsdl-token-ph").eq(a.index).find(".jcsdl-logic-token").addClass("error"),
this.scrollGuiToToken(c));d.animateIntoView(this.config.animate)},trigger:function(a,b){this.gui.trigger(a,b)},clearErrors:function(){this.gui.clearErrors();this.$gui.find(".error").removeClass("error")},initGui:function(a){var b=this,c=a.find(".jcsdl-advanced-gui"),d=null;this.$guiLeft=a.find(".jcsdl-advanced-gui-arrow.left");this.$guiRight=a.find(".jcsdl-advanced-gui-arrow.right");c.sortable({cursor:"move",placeholder:"jcsdl-token-ph",containment:a,appendTo:a,helper:"clone",tolerance:"intersect",
start:function(a,b){b.item.find(".jcsdl-logic-token").is(".bracket")&&c.find(".jcsdl-token-ph").addClass("bracket")},stop:function(a,c){var d=c.item.find(".jcsdl-logic-token");d.is(".filter")&&b.gui.highlightFilter(d.data("token"),!1)},sort:function(a,c){clearInterval(d);b.$guiRight.is(":visible")&&c.offset.left+c.helper.width()/2>=b.$guiRight.offset().left&&(d=setInterval(function(){b.scrollGui("right",50)},50));b.$guiLeft.is(":visible")&&c.offset.left<=b.$guiLeft.offset().left+c.helper.width()/
2&&(d=setInterval(function(){b.scrollGui("left",50)},50));var e=c.helper.find(".jcsdl-logic-token");e.is(".filter")&&b.gui.highlightFilter(e.html())},update:function(){clearInterval(d);setTimeout(function(){b.updateFromGui()},100)}});c.data("sortable").floating=!0;c.data("uiSortable").floating=!0;this.$insertBracketsButton.click(function(a){a.preventDefault();a.target.blur();if(e(this).is(".disabled"))return!1;b.addGuiToken(")",!0,!0);b.updateFromGui()});this.$guiRight.add(this.$guiLeft).bind("mousedown touchstart",
function(){var a=e(this).is(".right")?"right":"left";d=setInterval(function(){b.scrollGui(a,50)},50)}).bind("mouseup mouseleave touchend",function(){clearInterval(d)}).click(function(a){a.preventDefault();a.target.blur()});e(window).resize(function(){b.adjustGui()});return a},updateFromGui:function(){var a=this.getGuiTokens();try{this.validate(a)&&(this.clearErrors(),this.logicString=a.join(""))}catch(b){this.showError(b,!0)}},setGuiString:function(a){a=a||this.logicString;var b=this;a="string"===
typeof a||"number"===typeof a?this.tokenize(a):a;this.clearGui();e.each(a,function(a,d){b.addGuiToken(d,!1)})},getGuiString:function(a){a=a||this.$gui;var b="";a.find(".jcsdl-logic-token").each(function(){b+=""+e(this).data("token")});return b},getGuiTokens:function(a){a=a||this.$gui;var b=[];a.find(".jcsdl-logic-token").each(function(){b.push(e(this).data("token"))});return b},clearGui:function(){this.$gui.width(0).children().remove()},tokensMap:{"(":"bracketOpen",")":"bracketClose","&":"operatorAnd",
"|":"operatorOr",filter:"filter"},addGuiToken:function(a,b,c){b=b===h?!0:b;c=c===h?!1:c;var d=this,f="number"==typeof a?"filter":a;if(this.tokensMap[f]===h)throw'Invalid character "'+a+'" in the logic!';var g=this.getTemplate("logicToken_"+this.tokensMap[f]).data("token",a),j=e();"filter"==f&&g.html(a);f=e('<li class="jcsdl-logic-token-wrap" />').html(g).appendTo(this.$gui);this.$gui.width(this.$gui.width()+f.outerWidth(!0));if(")"==a&&c){j=this.getTemplate("logicToken_bracketOpen").data("token",
"(");a=this.getGuiTokens().reverse();var k=a.length,l=-1,m=null;e.each(a,function(a,b){")"==b?l++:"("==b&&l--;if(0==l&&0<=e.inArray(b,["&","|"])){if(null!==m&&b!==m)return k=a,!1;m=b}});a=this.$gui.children().eq(a.length-k);a=e('<li class="jcsdl-logic-token-wrap" />').html(j).insertBefore(a);this.$gui.width(this.$gui.width()+a.outerWidth(!0))}this.updateFromGui();this.adjustGui();b&&this.scrollGuiToToken(g);g.click(function(a){a.preventDefault();a.target.blur();"&"==g.data("token")?g.data("token",
"|").html("OR"):"|"==g.data("token")&&g.data("token","&").html("AND");d.updateFromGui()});g.add(j).find(".jcsdl-delete-token").click(function(a){a.preventDefault();a.target.blur();a.stopPropagation(!0);d.deleteGuiToken(g)});g.filter(".filter").hover(function(){d.gui.highlightFilter(e(this).data("token"))},function(){d.gui.highlightFilter(e(this).data("token"),!1)})},deleteGuiFilterToken:function(a){a=parseInt(a);var b=e(),c=e();this.$gui.children().each(function(){var c=e(this);if(parseInt(c.find(".jcsdl-logic-token").data("token"))===
a)return b=c,!1});if(!b.length)return!1;for(;"("==b.prev().find(".jcsdl-logic-token").data("token")&&")"==b.next().find(".jcsdl-logic-token").data("token");)b.prev().remove(),b.next().remove();b.prev().find(".jcsdl-logic-token.operator").length?c=b.prev():b.next().find(".jcsdl-logic-token.operator").length&&(c=b.next());b.remove();c.remove();this.updateFromGui()},deleteGuiToken:function(a){var b=a.is(".jcsdl-logic-token")?a.closest("li.jcsdl-logic-token-wrap"):a;if(!b.is("li.jcsdl-logic-token-wrap"))return!1;
a=b.find(".jcsdl-logic-token");if(a.is(".operator"))return!1;if(a.is(".filter")){var c=a.data("token"),d=!1;e.each(this.gui.filters,function(a,b){if(parseInt(b.id)==parseInt(c))return d=!0,!1});if(d)return!1}else if(a.is(".bracket")){var f=this.getGuiTokens(),g=this.$gui.children().index(b),j=null,k=0,l=0,m=")",h="(",n=")";")"==a.data("token")&&(f=f.reverse(),g=f.length-1-g,m="(",h=")",n="(");e.each(f,function(a,b){if(a==g)return l=k,!0;if(null!==l&&k==l&&a>g&&b==m)return j="("==m?f.length-1-a:a,
!1;b==h?k++:b==n&&k--});null!==j&&(a=this.$gui.children().filter(".jcsdl-logic-token-wrap").eq(j),this.$gui.width(this.$gui.width()-a.outerWidth(!0)),a.remove())}this.$gui.width(this.$gui.width()-b.outerWidth(!0));b.remove();this.updateFromGui();this.adjustGui()},_adjustGuiTimeout:null,adjustGui:function(){var a=this,b=0;this.$gui.children().each(function(){b+=parseInt(e(this).outerWidth(!0))});this.$gui.width(b);clearTimeout(this._adjustGuiTimeout);this._adjustGuiTimeout=setTimeout(function(){var b=
a.$gui.width(),d=a.$guiContainer.width();b>d?a.$guiRight.removeClass("off"):(a.$guiLeft.add(a.$guiRight).addClass("off"),a.scrollGuiToStart())},50)},scrollGui:function(a,b){b=b==h?50:b;var c=-1*Math.max(0,this.$gui.width()-this.$guiContainer.width()),d=0,f=this,d="number"===typeof a?a:parseInt(this.$gui.css("left"))+("right"==a?-30:30),d=Math.max(Math.min(0,d),c);this.$gui.stop(!0,!0).animate({left:d},b,function(){f.$guiLeft[0==d?"addClass":"removeClass"]("off");f.$guiRight[d==c?"addClass":"removeClass"]("off")})},
scrollGuiToStart:function(){this.scrollGui(0)},scrollGuiToEnd:function(){this.scrollGuiToToken(this.$gui.children().last())},scrollGuiToToken:function(a){a=a.is(".jcsdl-logic-token")?a.closest("li.jcsdl-logic-token-wrap"):a;if(!a.is("li.jcsdl-logic-token-wrap"))return!1;a=-1*a.position().left;parseInt(this.$gui.css("left"));this.scrollGui(a+140)},initManualInput:function(a){var b=this,c=null;a.keyup(function(){clearTimeout(c);c=setTimeout(function(){var c=a.val();try{b.validate(c)&&(b.clearErrors(),
b.logicString=c)}catch(f){b.showError(f,!0)}},500)});a.keydown(function(a){var b=a.which,c=a.shiftKey;8==b||(37<=b&&40>=b||46==b||48<=b&&57>=b&&!c||96<=b&&105>=b||219==b||57==b&&c||221==b||48==b&&c||32==b||55==b&&c||220==b&&c)||a.preventDefault()});a.keypress(function(a){13==a.which&&a.preventDefault()});return a},setManualString:function(a){a=a||this.logicString;a="string"===typeof a||"number"===typeof a?this.tokenize(a):a;this.$manualInput.val(a.join(" "))},getManualString:function(){return this.$manualInput.val()},
setLogic:function(a){this.logic=-1<e.inArray(a,[this.AND,this.OR])?a:this.ADVANCED;this.$logicOptions.filter('[data-logic="'+this.logic+'"]').click();var b="";if(this.logic==this.ADVANCED)b=a;else{var c=[];e.each(this.gui.filters,function(a,b){c.push(b.id)});b=c.join(this.logic==this.OR?"|":"&")}this.setLogicString(b)},getLogic:function(){return this.logic},setLogicString:function(a){this.logicString=""+a;this.setGuiString(a);this.setManualString(a)},appendFilterToLogicString:function(a,b){b=b||this.logic;
b=b==this.OR?"|":"&";this.setLogicString(this.logicString.length?this.logicString+b+a:a)},getLogicString:function(){return this.logic==this.ADVANCED?this.logicString:this.logic}}});;
