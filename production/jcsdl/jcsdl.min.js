...
JCSDL={};(function(){JCSDL.Loader={compnts:[],loaded:[],isLoaded:false,addComponent:function(f){if(typeof(f)=='function'){JCSDL.Loader.compnts.push(f);}},addLoaded:function(f){if(typeof(f)=='function'){JCSDL.Loader.loaded.push(f);}},load:function(){for(var i in JCSDL.Loader.compnts){JCSDL.Loader.compnts[i].apply(JCSDL,[window.jQuery]);}
for(var i in JCSDL.Loader.loaded){JCSDL.Loader.loaded[i].apply(JCSDL,[]);}
JCSDL.Loader.isLoaded=true;},timeout:function(nc){setTimeout(function(){if(window.jQuery){if(nc)window.jQuery.noConflict();JCSDL.Loader.load();}else{JCSDL.Loader.timeout();}},100);}};JCSDL.onLoad=function(f){if(typeof(f)!=='function')return false;if(JCSDL.Loader.isLoaded){f.apply(JCSDL,[]);}else{JCSDL.Loader.addLoaded(f);}};})();
...
return $view;},createTextOperatorsSelectView:function($view,field,inputType,$inputView){var self=this;$('.jcsdl-dropdown').remove();$view.addClass('text');var $input=$inputView.find('input.orig');var $select=this.getTemplate('textOperatorsSelect');var $dropdown=this.getTemplate('textOperatorsDropdown');var inputConfig=(typeof(this.definition.inputs[inputType])!=='undefined')?this.definition.inputs[inputType]:{};var allowedOperators=(typeof(inputConfig.operators)!=='undefined')?inputConfig.operators:[];$.each(field.operators,function(i,operator){if(allowedOperators.length>0&&($.inArray(operator,allowedOperators)==-1))return true;var $operatorView=self.createTextOperatorOptionView(operator);$dropdown.append($operatorView);});$view.html($select).insertAfter($inputView);$dropdown.hide().appendTo('body');if(field.cs){var $csView=this.getTemplate('caseSensitivity');$csView.bind('click touchstart',function(ev){ev.preventDefault();ev.target.blur();$(this).toggleClass('selected');}).tipsy({gravity:'s',offset:9});$csView.prependTo($view);$select.addClass('has-cs');}
...
return this.parser.getFieldInfo(this.currentFilterTarget,fieldsPath);},getIconForField:function(field,fieldInfo){return(typeof(fieldInfo.icon)!=='undefined')?fieldInfo.icon:field;},getFilterIndexByElement:function($filterRow){return this.$filtersList.find('.jcsdl-filter').index($filterRow);}};$.fn.jcsdlGui=function(options){function get($el){var gui=$el.data('jcsdlGui');if(!gui){gui=new JCSDL.GUI($el,options);$el.data('jcsdlGui',gui);}
return gui;}
if(typeof(options)=='string'){if($.inArray(options,['loadJCSDL'])>=0){var argmns=[];$.each(arguments,function(i,arg){if(i==0)return true;argmns.push(arg);});this.each(function(){var gui=get($(this));gui[options].apply(gui,argmns);});}
return this;}
