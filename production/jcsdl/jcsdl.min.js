...
return bounds;};}
if(jcsdlMapsCurrentGui&&jcsdlMapsCurrentCallback){setTimeout(function(){jcsdlMapsCurrentCallback.apply(jcsdlMapsCurrentGui.inputs,jcsdlMapsCurrentCallbackArgs);},jcsdlMapsCurrentGui.config.animate+10);}};(function($){$.extend(String.prototype,{truncate:function(l,a,h){l=l||72;a=a||'...';if(this.length<=l)return this.valueOf();s=this.substr(0,l);if(!h){s=s.substr(0,s.lastIndexOf(' '));}
...
return $option;},createInputSelectionView:function(inputs){var self=this;var $inputView=this.getTemplate('inputSelect');var $inputSelect=$inputView.find('.jcsdl-filter-target-field-input');$.each(inputs,function(i,name){var $inputOptionView=self.createOptionForInput(name);$inputOptionView.appendTo($inputSelect);});return $inputView;},createOptionForInput:function(name){var $option=this.getTemplate('inputSelectOption');$option.data('name',name).addClass('icon-'+name+' input-'+name).html(name);return $option;},createValueInputView:function(field,inputType){var self=this;inputType=inputType||field.input;var $valueView=this.getTemplate('valueInput');if(typeof(this.inputs[inputType])=='undefined')return $valueView;$valueView.find('.jcsdl-target-help a').tipsy({gravity:'s',offset:10}).click(function(ev){ev.preventDefault();ev.target.blur();var target=self.currentFilterTarget+'.'+self.currentFilterFieldsPath.join('.').replace(/-/g,'.');var popup=$.jcsdlPopup({title:target});popup.setContent('<p>'+target+'</p>');});var $inputView=this.inputs.exec(inputType,'init',[field]);var $input=$inputView.find('input:first');var $valueInput=$valueView.find('.jcsdl-filter-value-input-field');$valueInput.data('inputType',inputType).html($inputView);var $operatorsListView=$valueView.find('.jcsdl-filter-value-input-operators');if(inputType=='text'){this.createTextOperatorsSelectView($operatorsListView,field,inputType,$valueInput);}else{this.createOperatorsSelectView($operatorsListView,field,inputType,$valueInput);}
...
return this.parser.getFieldInfo(this.currentFilterTarget,fieldsPath);},getIconForField:function(field,fieldInfo){return(typeof(fieldInfo.icon)!=='undefined')?fieldInfo.icon:field;},getFilterIndexByElement:function($filterRow){return this.$filtersList.find('.jcsdl-filter').index($filterRow);}};$.fn.jcsdlGui=function(options){function get($el){var gui=$el.data('jcsdlGui');if(!gui){gui=new JCSDLGui($el,options);$el.data('jcsdlGui',gui);}
return gui;}
...
return this;}
this.each(function(){get($(this));});return this;};})(window.jQuery);
