...
JCSDL={};(function(){JCSDL.Loader={compnts:[],loaded:[],isLoaded:false,addComponent:function(f){if(typeof(f)=='function'){JCSDL.Loader.compnts.push(f);}},addLoaded:function(f){if(typeof(f)=='function'){JCSDL.Loader.loaded.push(f);}},load:function(){var $=window.jQuery;$.each(JCSDL.Loader.compnts,function(){this.apply(JCSDL,[$]);});$.each(JCSDL.Loader.loaded,function(){this.apply(JCSDL,[]);});JCSDL.Loader.isLoaded=true;},timeout:function(nc){setTimeout(function(){if(window.jQuery){if(nc)window.jQuery.noConflict();JCSDL.Loader.load();}else{JCSDL.Loader.timeout();}},100);}};JCSDL.onLoad=function(f){if(typeof(f)!=='function')return false;if(JCSDL.Loader.isLoaded){f.apply(JCSDL,[]);}else{JCSDL.Loader.addLoaded(f);}};})();
...
JCSDL.Loader.addComponent(function($){JCSDL.Parser=function(gui){this.gui=gui;};JCSDL.Parser.prototype={v:'1.0',parseJCSDL:function(code){var lines=code.split("\n");var masterLine=lines.shift();var endLine=lines.pop();if(!this.verifyJCSDL(masterLine,lines)){this.error('The given JCSDL did not verify!',code);return false;}
...
JCSDL.Loader.addComponent(function($){var JCSDLCarousel=function($el,options){var self=this;this.select=options.select;this.$carousel=$el.find('.jcsdl-carousel');this.$carouselWrap=this.$carousel.closest('.jcsdl-carousel-wrap');this.$carouselItems=this.$carousel.find('.jcsdl-carousel-item');this.$scrollLeft=this.$carouselWrap.siblings('.jcsdl-carousel-scroll.left');this.$scrollRight=this.$carouselWrap.siblings('.jcsdl-carousel-scroll.right');this.itemsCount=this.$carouselItems.filter(':visible').length;var $exampleItem=this.$carouselItems.eq(0),hideExample=($exampleItem.is(':visible'))?false:true;$exampleItem.show();this.itemWidth=$exampleItem.outerWidth(true);this.itemMargin=parseInt($exampleItem.css('marginRight'));this.$carouselWrap.css({maxWidth:this.calculateWrapWidth(),height:$exampleItem.outerHeight(true)});this.$carousel.css({position:'relative',width:this.itemWidth*this.itemsCount,height:$exampleItem.outerHeight(true)});this.margin=this.calculateCenterMargin();if(hideExample)$exampleItem.hide();var $selected=this.$carouselItems.filter('.selected');this.selectedIndex=($selected.length==1)?$selected.prevAll(':visible').length:Math.floor(this.itemsCount/2);if(this.itemsCount%2==0){this.selectedIndex=this.selectedIndex-0.5;}
...
return this;}
options=$.extend({},{select:function(){},expand:false,speed:200},options);this.each(function(){get($(this));});return this;};});JCSDL.Loader.addComponent(function($){$.fn.jcsdlNumberMask=function(fl,arr){this.each(function(){$(this).keydown(function(ev){if((ev.which==190||ev.which==110)&&(($(this).val().indexOf('.')>=0)||!fl)){ev.preventDefault();return;}
if(ev.shiftKey||ev.altKey){ev.preventDefault();return;}else if(ev.which==46||ev.which==8||ev.which==9||ev.which==27||ev.which==190||ev.which==110||(ev.which==188&&arr)||(ev.which==65&&ev.ctrlKey===true)||(ev.which>=35&&ev.which<=39)||ev.which==13){return;}else{if((ev.which<48||ev.which>57)&&(ev.which<96||ev.which>105)){ev.preventDefault();}}});});return this;};});JCSDL.Loader.addComponent(function($){$.fn.jcsdlOrigVal=$.fn.val;$.fn.val=function(value){if(!this[0])return undefined;var $self=$(this[0]);if($self.data('jcsdlTagInput')){if(typeof(value)!=='undefined'){$self.jcsdlOrigVal(value);$self.trigger('change');return $self;}}
...
this.$mainView.find('.jcsdl-filters-logic input[name="logic"]').change(function(ev){self.logic=$(this).val();self.trigger('logicChange',[self.logic]);});this.$mainView.find('.jcsdl-mainview-mode .jcsdl-mainview-mode-option').bind('click touchstart',function(ev){ev.preventDefault();ev.target.blur();var $item=$(this),mode=$item.attr('data-mode');self.$filtersList.removeClass('expanded collapsed');self.$filtersList.addClass(mode);self.$mainView.find('.jcsdl-mainview-mode .jcsdl-mainview-mode-option').removeClass('active');$item.addClass('active');self.trigger('viewModeChange',[mode]);});this.$mainView.find('.jcsdl-add-filter').bind('click touchstart',function(ev){ev.preventDefault();ev.target.blur();self.showFilterEditor();});this.$saveButton.unbind('.jcsdl').bind('click.jcsdl touchstart.jcsdl',function(ev){ev.preventDefault();ev.target.blur();var code=self.returnJCSDL();self.trigger('save',[code]);});this.$cancelButton.unbind('.jcsdl').bind('click.jcsdl touchstart.jcsdl',function(ev){ev.preventDefault();ev.target.blur();self.trigger('cancel');});},reset:function(){this.init();},loadJCSDL:function(code){this.init();var self=this;var parsed=this.parser.parseJCSDL(code);if(parsed===false){this.showError('Invalid JCSDL input!',code);return;}
this.logic=(parsed.logic=='AND')?'AND':'OR';this.filters=parsed.filters;this.$mainView.find('.jcsdl-filters-logic input[name="logic"][value="'+this.logic+'"]').click();$.each(this.filters,function(i,filter){var $filterRow=self.createFilterRow(filter);$filterRow.appendTo(self.$filtersList);});if(this.config.cancelButton){this.$cancelButton.show();}},returnJCSDL:function(){return this.parser.getJCSDLForFilters(this.filters,this.logic);},adjust:function(){this.$currentFilterStepsView.find('.jcsdl-step').jcsdlCarousel('adjust');},trigger:function(name,args){args=args||[];if(typeof(this.config[name])=='function'){return this.config[name].apply(this,args);}
...
var $valueView=this.createValueInputView(field);this.addFilterStep('value',$valueView,(firstRemoved!='value'));this.addTargetHelpToStep($stepView);this.trigger('targetSelect',[this.getCurrentPath()]);},didSelectInput:function(inputName,$inputView,$stepView){var inputPosition=$inputView.data('position');var firstRemoved=this.removeFilterStepsAfterPosition($inputView.closest('.jcsdl-step').data('number'));$stepView.removeClass('selected-final').find('.jcsdl-target-help').remove();var $valueView=this.createValueInputView(this.getFieldInfoAtCurrentPath(),inputName);this.addFilterStep('value',$valueView,(firstRemoved!='value'));this.addTargetHelpToStep($stepView);},addTargetHelpToStep:function($stepView){var self=this;this.getTemplate('targetHelp').appendTo($stepView);$stepView.find('.jcsdl-target-help').click(function(ev){ev.preventDefault();ev.target.blur();var info=self.getFieldInfoAtCurrentPath(),target=self.currentFilterTarget,targetInfo=self.parser.getTargetInfo(target);path=self.currentFilterFieldsPath.join('-');var title=targetInfo.name;var cPath=[];$.each(self.currentFilterFieldsPath,function(i,name){cPath.push(name);title+=' &raquo; '+self.parser.getFieldInfo(target,cPath).name;});var cacheName=target+'.'+path;var popup=$.jcsdlPopup({title:title});var url=self.config.targetHelpUrl.apply(self,[cacheName.replace(/-/g,'.'),info]);if(typeof(self.jsonpCache.targets[cacheName])=='undefined'){$.ajax({url:url,type:'GET',async:false,jsonpCallback:'jcsdlJSONP',contentType:'application/json',dataType:'jsonp',success:function(data){if(!data||!data.html){popup.setContent('<p>No documentation available.</p>');popup.reposition();return;}
popup.setContent(data.html);popup.reposition();self.jsonpCache.targets[cacheName]=data.html;},error:function(){popup.setContent('<p>No documentation available.</p>');popup.reposition();}});}else{popup.setContent(self.jsonpCache.targets[cacheName]);popup.reposition();}});},didSubmitFilter:function(){var fieldInfo=this.getFieldInfoAtCurrentPath();var operator=(fieldInfo.input=='text')?this.$currentFilterView.find('.jcsdl-filter-value-input-operators .jcsdl-operators-select').data('operator'):this.$currentFilterView.find('.jcsdl-filter-value-input-operators .operator.selected').data('name');if(typeof(operator)=='undefined'){this.showError('You need to select an operator!');return;}
...
$filterRow.data('filter',filter);$filterRow.find('.edit').bind('click touchstart',function(ev){ev.preventDefault();ev.target.blur();var index=self.getFilterIndexByElement($filterRow);self.showFilterEditor(filter,index);});$filterRow.find('.delete').bind('click touchstart',function(ev){ev.preventDefault();ev.target.blur();var index=self.getFilterIndexByElement($filterRow);self.deleteFilter(index);});return $filterRow;},createTargetSelectView:function(){var self=this;var $targetSelectView=this.getTemplate('target');var $targetSelect=$targetSelectView.find('.jcsdl-filter-target');$.each(this.definition.targets,function(name,target){var $targetView=self.createOptionForTarget(name,target);$targetView.appendTo($targetSelect);});return $targetSelectView;},createOptionForTarget:function(name,target){var $option=this.getTemplate('targetOption');$option.data('name',name);$option.data('target',target);$option.addClass('target-'+name);if($.inArray(name,this.config.hideTargets)>=0){$option.addClass('jcsdl-option-hidden');if(!(this.currentFilterIndex>=0)||!this.filters[this.currentFilterIndex]||(this.filters[this.currentFilterIndex].target!==name)){$option.hide();}}
return $option;},createFieldSelectionView:function(fields){var self=this;var $fieldView=this.getTemplate('field');var fieldPosition=this.currentFilterFieldsPath.length;$fieldView.data('position',fieldPosition);$fieldView.addClass('field-select-'+fieldPosition);var currentPath=this.currentFilterTarget+(this.currentFilterFieldsPath.length>0?'.'+this.currentFilterFieldsPath.join('.'):'');var hidden=false;var path=$.merge([this.currentFilterTarget],this.currentFilterFieldsPath);var cPath='';$.each(path,function(i,target){cPath=cPath+(i>0?'.':'')+target;if($.inArray(cPath,self.config.hideTargets)>=0){hidden=true;return true;}});var $fieldSelect=$fieldView.find('.jcsdl-filter-target-field');$.each(fields,function(name,field){var $fieldView=self.createOptionForField(name,field,currentPath,hidden);$fieldView.appendTo($fieldSelect);});return $fieldView;},createOptionForField:function(name,fieldInfo,parentPath,hidden){var self=this;var $option=this.getTemplate('fieldOption');$option.data('name',name);$option.data('field',fieldInfo);$option.html(fieldInfo.name);$option.addClass('icon-'+this.getIconForField(name,fieldInfo));$option.addClass('field-'+name);if(!hidden){var path=parentPath+'.'+name;if($.inArray(path,self.config.hideTargets)>=0){hidden=true;}else{path=path.split('-');var sectionPath='';$.each(path,function(i,section){sectionPath+=(i>0?'.':'')+section;if($.inArray(sectionPath,self.config.hideTargets)>=0){hidden=true;return true;}});}}
if(hidden){$option.addClass('jcsdl-option-hidden');}
if(hidden&&(this.currentFilterIndex>=0)&&this.filters[this.currentFilterIndex]){var cFilterPath=this.filters[this.currentFilterIndex].target+'.'+this.filters[this.currentFilterIndex].fieldPath.join('.').replace(/-/g,'.');var cPath=parentPath+'.'+name;if(cFilterPath==cPath){hidden=false;}}
if(hidden){$option.hide();}
return $option;},createInputSelectionView:function(inputs){var self=this;var $inputView=this.getTemplate('inputSelect');var $inputSelect=$inputView.find('.jcsdl-filter-target-field-input');$.each(inputs,function(i,name){var $inputOptionView=self.createOptionForInput(name);$inputOptionView.appendTo($inputSelect);});return $inputView;},createOptionForInput:function(name){var $option=this.getTemplate('inputSelectOption');$option.data('name',name).addClass('icon-'+name+' input-'+name).html(name);return $option;},createValueInputView:function(field,inputType){var self=this;inputType=inputType||field.input;var $valueView=this.getTemplate('valueInput');if(typeof(this.inputs[inputType])=='undefined')return $valueView;var $inputView=this.inputs.exec(inputType,'init',[field]);var $input=$inputView.find('input:first');var $valueInput=$valueView.find('.jcsdl-filter-value-input-field');$valueInput.data('inputType',inputType).html($inputView);var $operatorsListView=$valueView.find('.jcsdl-filter-value-input-operators');if(inputType=='text'){this.createTextOperatorsSelectView($operatorsListView,field,inputType,$valueInput);}else{this.createOperatorsSelectView($operatorsListView,field,inputType,$valueInput);}
...
return true;},getValueFromField:function($view,fieldInfo,operator){var inputType=$view.data('inputType');if(typeof(this.inputs[inputType])=='undefined')return'';return this.inputs.exec(inputType,'getValue',[$view,fieldInfo,operator]);},getTemplate:function(name){if(typeof(JCSDL.GUITemplates[name])!=='undefined'){return JCSDL.GUITemplates[name].clone();}
...
return this.parser.getFieldInfo(this.currentFilterTarget,fieldsPath);},getCurrentPath:function(){return this.currentFilterTarget+(this.currentFilterFieldsPath.length>0?'.':'')+this.currentFilterFieldsPath.join('.').replace(/-/g,'.');},getIconForField:function(field,fieldInfo){return(typeof(fieldInfo.icon)!=='undefined')?fieldInfo.icon:field;},getFilterIndexByElement:function($filterRow){return this.$filtersList.find('.jcsdl-filter').index($filterRow);}};$.fn.jcsdlGui=function(options){function get($el){var gui=$el.data('jcsdlGui');if(!gui){gui=new JCSDL.GUI($el,options);$el.data('jcsdlGui',gui);}
return gui;}
